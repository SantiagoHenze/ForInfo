...
$select="SELECT usu.name, dis.tags, dis.fecha, dis.discusion, dis.contenido, dis.imagen FROM usuarios AS usu INNER JOIN discusiones AS dis ON usu.ID_Usuario = dis.id_usuario WHERE usu.name LIKE '$busqueda' OR dis.tags LIKE '$busqueda' OR dis.fecha LIKE '$busqueda' OR dis.discusion LIKE '$busqueda' OR dis.contenido LIKE '$busqueda' OR dis.tags LIKE '$busqueda';";
$ejecucion=mysqli_query($con, $select);
$select2="SELECT usu.name, dis.discusion, dis.fecha FROM usuarios AS usu INNER JOIN discusiones AS dis ON usu.ID_Usuario = dis.id_usuario;";    
        ...
        <div class="comentarios">
            ...
            <?php while ($contenido=mysqli_fetch_array($ejecucion2)) { ?>
                <div class="comentario">
                    ...
                    <p><?php echo $contenido['discusion']; ?></p>
                </div>
            <?php } ?>
